<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="加积分备注" prop="jiajifenbeizhu">
          <el-input v-model="ruleForm.jiajifenbeizhu" 
              placeholder="加积分备注" clearable  :readonly="ro.jiajifenbeizhu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="加积分备注" prop="jiajifenbeizhu">
              <el-input v-model="ruleForm.jiajifenbeizhu" 
                placeholder="加积分备注" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.jifen = true;
          ...
          if(o=='jiajifenbeizhu'){
            this.ruleForm.jiajifenbeizhu = obj[o];
	    this.ro.jiajifenbeizhu = true;
            continue;
          }
        }
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jiajifenjilu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jiajifenjiluCrossAddOrUpdateFlag = false;
                  this.parent.search();
