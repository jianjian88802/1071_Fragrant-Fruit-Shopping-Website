<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="收货人电话" prop="shouhuorendianhua">
          <el-input v-model="ruleForm.shouhuorendianhua" 
              placeholder="收货人电话" clearable  :readonly="ro.shouhuorendianhua"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="收货人电话" prop="shouhuorendianhua">
              <el-input v-model="ruleForm.shouhuorendianhua" 
                placeholder="收货人电话" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  computed: {
    zongjine:{
      get: function () {
        return 1*this.ruleForm.shuliang*this.ruleForm.jiage*this.ruleForm.zhekou
      }
    },
  },
  created() {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shouhuodizhi = true;
          ...
          if(o=='shouhuorendianhua'){
            this.ruleForm.shouhuorendianhua = obj[o];
	    this.ro.shouhuorendianhua = true;
            continue;
          }
    ...
    info(id) {
      this.$http({
        url: `huiyuangoumaidingdan/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `huiyuangoumaidingdan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.huiyuangoumaidingdanCrossAddOrUpdateFlag = false;
                  this.parent.search();
