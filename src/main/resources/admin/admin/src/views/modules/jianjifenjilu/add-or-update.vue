<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="水果编号" prop="shuiguobianhao">
          <el-select  @change="shuiguobianhaoChange" v-model="ruleForm.shuiguobianhao" placeholder="请选择水果编号">
            <el-option
                v-for="(item,index) in shuiguobianhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.shuiguobianhao" label="水果编号" prop="shuiguobianhao">
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/shuiguo/shuiguobianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.shuiguobianhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    shuiguobianhaoChange () {
      this.$http({
        url: `follow/shuiguo/shuiguobianhao?columnValue=`+ this.ruleForm.shuiguobianhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.jifen){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jianjifenjilu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jianjifenjiluCrossAddOrUpdateFlag = false;
                  this.parent.search();
